...
module.exports = {
    ...
    execute: async (message, args, client) => {
        ...
        const attachment = new MessageAttachment(`commands/komendy/reactions/graphics/run/run_${rng}.gif`)
        ...
        if (args[0] && args[0] != "help") {
            ...
            try {
                message.channel.send(`${message.author} runing for <@${tag_user}>`);
        ...
        try {
            message.channel.send(`${message.author} runing`)
            message.channel.send({ files: [attachment] });
            reactions_handler(action_type,message)
        } catch (error) {
            console.log(error)
